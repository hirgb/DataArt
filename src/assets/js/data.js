function loadData(echarts, data, element) {
    let chart = echarts.init(element);
    let option = {
        title: {
            text: data.name + '-' + data.code,
            link: `/#/stock/${data.code}`,
            target: 'self',
            textStyle: {
                color: '#eee',
            }
...
function loadOverViewData(echarts, data, element) {
    let chart = echarts.init(element);
    let dataXaxis = data.map((item) => {
        return item['name'];
    });
    let dataIn = data.map((item) => {
        return item['in'] / 10000;
    });
    let dataOut = data.map((item) => {
        return item['out'] / 10000;
    });
    let dataPurein = data.map((item) => {
...
function loadIndustryData(echarts, data, element) {
    let chart = echarts.init(element);
...
function loadConceptData(echarts, data, element) {
    let chart = echarts.init(element);
    let xAxisData = data.name;
    let value = data.value;
    let option = {
...
function loadAreaData(echarts, data, element) {
    let chart = echarts.init(element);
    let xAxisData = data.name;
    let value = data.value;
    let option = {
...
function loadZdfbData(echarts, data, element) {
    let chart = echarts.init(element);
    let option = {
        title: {
            // text: '涨跌分布',
...
function loadDppjData(echarts, data, element) {
    let a = [];
    let obj = {
        value: data.value
    };
...
function genFavoriteChartOption(echarts) {
    let option = {
        title: {
            text: '',
            link: '',
